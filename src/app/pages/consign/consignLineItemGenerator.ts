...
function generateMockConsignLineItems(
  count: number
): ConsignSaleLineItemsListResponse[] {
  return Array.from({ length: count }, (_, index) => ({
    ...
    confirmedPrice:
      Math.random() > 0.5 ? Math.floor(Math.random() * 1000000) + 50000 : null, // 50% chance of having a confirmed price
    productName: `${getRandomElement(brands)} ${getRandomElement(
      productTypes
    )}`,
